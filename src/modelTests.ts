...
export async function runModelTests(forwardedArgv: string[]): Promise<void> {
	const argv = syntheticArgv(forwardedArgv);
	const configPath = parseArg(argv, "--config") ?? "config/servers.json";
	const casesPath =
		parseArg(argv, "--cases") ?? "prompts/model-test-cases.json";
	...
	console.error(
		`[model-tests] provider=${config.model.provider} model=${config.model.modelName} cases=${cases.length}`,
	);
	let passed = 0;
	let failed = 0;
	...
	for (const testCase of cases) {
		...
		try {
			...
			if (!text) {
				failed += 1;
				console.error(
					`[model-tests] FAIL name=${testCase.name} elapsedMs=${elapsedMs} reason=empty_text`,
				);
				continue;
			}
			passed += 1;
			console.error(
				`[model-tests] PASS name=${testCase.name} elapsedMs=${elapsedMs} chars=${text.length}`,
			);
			console.log(`\n=== ${testCase.name} ===\n${text}\n`);
		} catch (error) {
			failed += 1;
	...
	console.error(
		`[model-tests] summary passed=${passed} failed=${failed} total=${cases.length}`,
	);
	if (failed > 0) {
		process.exit(1);
	}
...
async function main(): Promise<void> {
	await runModelTests(process.argv.slice(2));
}
