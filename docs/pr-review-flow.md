...
\* You must provide a non-empty diff either via `--diff` or by piping stdin. Relative paths resolve from your current shell directory (run from repo root for the examples below).
...
### Source Code API v2: token, cookie, or Basic + `--branch` / `--commit` (quality POST)
...
**4 — HTTP Basic only (no token/cookie): fetch diff and POST review**

```bash
export SOURCE_CODE_API_BASIC_USER="gateway-user"
export SOURCE_CODE_API_BASIC_PASSWORD="gateway-pass"
bun run review:pr -- \
  --provider sourceCodeApi \
  --base-url "https://scm.example.com/app/sourcecode/api/api/v2" \
  --project-key "ACME/platform" \
...
The default issues POST body includes **`repoTask` and `repo_task`** (same object) **inside** `data` and **again** at the **root** — some backends only deserialize the snake_case key even when errors refer to `repoTask`. Task `branch` / `commit` match `--branch` / `--commit`; override the task title with `--quality-repo-task-name`.
